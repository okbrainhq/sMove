#!/usr/bin/python3
"""Small bounded, clearance-aware route completion for the three IMU ECO nets.
...
def connect(net,start,end):
 ...
 for f in b.GetFootprints():
  for pad in f.Pads():
   ...
   for l in obstacles:
    if pad.IsOnLayer(l):add(l,pad.GetEffectiveShape(l))
 ...
 def safe(x,y,layer,via=False):
  xx=x*S;yy=y*S;rr=.378 if via else .224
  if not(100.25+rr<xx<124.75-rr and 100.25+rr<yy<138.75-rr):return False
  if xx+rr>124.2 and 111.2-rr<yy<122.4+rr:return False
  if xx+rr>107.45 and xx-rr<117.55 and yy+rr>132.75:return False
 ...
 for u,v in zip(verts,verts[1:]):
  if u[2]!=v[2]:
   ...
   t=p.PCB_TRACK(b);t.SetStart(V(u[0]*S,u[1]*S));t.SetEnd(V(v[0]*S,v[1]*S));t.SetLayer(u[2]);t.SetWidth(p.FromMM(.127));t.SetNet(nn);b.Add(t)
