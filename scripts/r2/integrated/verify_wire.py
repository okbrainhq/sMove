...
import pcbnew as p
ROOT=Path(__file__).resolve().parents[3];H=ROOT/'PCB/main';C=ROOT/'PCB/imu-carrier';BASE='4cb3c32193bda18d624b7974b65162a7a44f69a0'
def old(path):return sp.check_output(['git','show',BASE+':'+path],text=True)
from sexp import *
O=ROOT/'docs/revision-r2/solder-wire/validation';O.mkdir(parents=True,exist_ok=True);checks=[]
...
before=pins(O/'baseline.xml');removed={('J2','MP')};new_nc=set()
...
(O/'net-migrations.json').write_text(json.dumps(dict(intentional_removals=sorted(removed),restored_nc=sorted(new_nc),retained=migrations),indent=2)+'\n')
...
check('single_integrated_population',set(fps)==set(parts)|{'H1','H2'} and len(fps)==49 and sum(v['fitted'] for v in parts.values())==46)
for r,v in parts.items():
 ...
 if r in oldparts and r!='J2':check('preserved_main_MPN:'+r,v['mpn']==oldparts[r]['mpn'] and v.get('jlc')==oldparts[r].get('jlc'))
 if r.startswith('TP'):
...
from solder_wire import HOLES,OUTLINE
...
# RF geometry is exactly unchanged from current-main baseline.
...
bomrefs=[r for row in rows for r in row['Designator'].split(',')];check('BOM_46_exact',set(bomrefs)=={r for r,v in parts.items() if v['fitted']} and len(bomrefs)==46)
# Intentional J2 mechanical-to-wire migration, not a new connector or testpoint.
f=fps['J2'];pads=sorted(f.Pads(),key=lambda x:x.GetNumber())
check('J2_two_PTH_wire_pads',len(pads)==2 and all(a.GetAttribute()==p.PAD_ATTRIB_PTH for a in pads))
check('J2_standard_centre_pitch',abs(math.dist(p.ToMM(pads[0].GetPosition()),p.ToMM(pads[1].GetPosition()))-2.54)<1e-6)
check('J2_hole_and_annulus',all(tuple(p.ToMM(a.GetDrillSize()))==(1.,1.) and tuple(p.ToMM(a.GetSize()))==(2.,2.) for a in pads))
check('J2_no_paste_no_fitted_header',all(not a.IsOnLayer(p.F_Paste) and not a.IsOnLayer(p.B_Paste) for a in pads) and f.IsExcludedFromBOM() and f.IsExcludedFromPosFiles() and not parts['J2']['fitted'] and not parts['J2']['mpn'] and not parts['J2']['jlc'])
check('J2_polarity_exact',pads[0].GetNetname()==before[('J2','1')] and pads[1].GetNetname()==before[('J2','2')]=='GND')
check('J2_mask_web',all(abs(p.ToMM(a.GetLocalSolderMaskMargin())-.05)<1e-6 for a in pads) and abs(2.54-2.-2*.05-.44)<1e-6)
for text in ['BAT+','BAT-']:check('wire_silk:'+text,any(isinstance(t,p.PCB_TEXT) and t.GetText()==text and t.GetLayer()==p.F_SilkS for t in b.GetDrawings()))
check('wire_documented_envelope_not_invented_gauge',json.loads((H/'interface.json').read_text())['battery_wire']['wire_gauge_supplied'] is None)
check('outline_compacted',json.loads((H/'interface.json').read_text())['outline_native_xy_mm']==[list(x) for x in OUTLINE])
check('actual_population_compacted',all(parts[r]['placement']!=oldparts[r]['placement'] for r in ['SW3','D1','R11','R12','R13']))
for r in ['U1','U2','U3','U4','U5','U6','U8','U9','C1','C2','C3','C4','C5','C6','C7','C8','C9','C10','C11','C13','J1']:
 check('unchanged_critical_pose:'+r,parts[r]['placement']==oldparts[r]['placement'])
oldinterface=json.loads(old('PCB/main/interface.json'));interface=json.loads((H/'interface.json').read_text())
for key in ['accel_gyro_to_body','mag_to_body','native_rotation_deg','side','evidence']:
 check('signed_IMU_axis:'+key,interface['sensor_frame'][key]==oldinterface['sensor_frame'][key])
