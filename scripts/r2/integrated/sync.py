...
j['components']['J2']=dict(ref='J2',mpn='PCB PTH wire pads - not fitted hardware',fitted=False,body_aabb_native_xy_mm=bbox(f.GetCourtyard(p.F_CrtYd).BBox()),courtyard_native_xy_mm=bbox(f.GetCourtyard(p.F_CrtYd).BBox()),anchor_native_xy_mm=list(p.ToMM(f.GetPosition())),rotation_ccw_deg=f.GetOrientationDegrees(),z_mm_from_board_bottom=[0,1])
j['anchors']['J2']={'center_native_xy_mm':list(p.ToMM(f.GetPosition())),'pins':{a.GetNumber():{'native_xy_mm':list(p.ToMM(a.GetPosition())),'net':a.GetNetname()} for a in f.Pads()},'wire_exit':'Through board beside shielded module body, then WEST side descent behind RF boundary; never through antenna overhang'}
...
j.update(schema='smove.imu-support.interface.v5',geometry_sha256=hashlib.sha256((H/'smove-r2-main.kicad_pcb').read_bytes()).hexdigest(),outline_native_xy_mm=OUTLINE,board_size_mm=[x1-x0,y1-y0],status='30mm PCB / PTH BATTERY WIRES / TOP IMU / ONE M3 + POSITIVE KEYS; PROTOTYPE ONLY')
...
j['battery_wire']={'pitch_centre_mm':2.54,'pad_diameter_mm':2.0,'finished_hole_target_mm':1.0,'finished_hole_acceptance_mm':[0.9,1.1],'tinned_bundle_max_diameter_mm':0.7,'insulation_max_diameter_mm':1.2,'nominal_annular_ring_mm':0.5,'mask_expansion_mm':0.05,'copper_gap_mm':0.54,'mask_web_mm':0.44,'wire_gauge_supplied':None,'wire_selection':'Engineering envelope only; measure tinned strand bundle and insulated OD, qualify rated current/flex/temperature; no exact gauge supplied','strain_relief':'West-wall bridge, two guide bores and two nylon lacing bores; paired OD<=1.2mm leads, R2 swept bends; pull/flex test mandatory, solder is not strain relief','polarity':{'1':'BAT+ protected PACK_P','2':'BAT- GND'}}
j['antenna_3d']['housing_recommendation_status']='15mm clearance in ALL directions is recommended by Espressif but NOT met by the existing compact housing/under-board pack. The retained overhang forward/side exclusion is a geometric constraint, not proof of RF compliance. No wire or pack may enter it; qualify final RF on body.'
j['mounting']['sensor_warning']='Nearer support shortens unsupported PCB span, not an accuracy guarantee. Control preload and coplanarity; no contact with U2. Magnetometer: prefer verified low-magnetic hardware; even nominal stainless can be magnetic. Qualify bias with installed hardware and charging currents.'
j['bottom_envelope'].pop('J3',None)
j['bottom_envelope']['J2']='PTH leads from underside; trim top protrusion <=0.6mm, insulate underside down to wire insulation, route above divider then through the relieved west edge into the side lacing bay'
from pcb_review_policy import apply as apply_pcb_review_policy
apply_pcb_review_policy(j)
