...
> **Schematic ECO 2026-09-15 (SCHEMATIC ONLY — no PCB change, no routing).** Added **SW1**, an off-board pack toggle switch in series in the battery + lead (hard "True OFF", ≥0.5 A / <50 mΩ, wired to the same 2.54 mm wire-pad footprint as J2); routed the charger's open-drain **CHG_N** to **U1 GPIO6** with a 100k pull-up (**R24**); and changed **U4** to **XC6220B331MR-G** (SOT-25, LCSC C86534) with a new footprint `SOT-25_XC6220`. The AP2112K's SOT-23-5 pin map is identical (1=VIN 2=VSS 3=CE 4=NC 5=VOUT), so no rewire was needed. ERC is **0 violations**, matching the pre-change baseline; the board, `dist/` and enclosure data are unchanged and stale until a forward update (F8) plus placement is done. See [Main PCB status](PCB/main/README.md) and [radio/battery architecture](docs/revision-r2/integrated/RADIO-BATTERY.md).
