...
refs=[o for o in doc.Objects if getattr(o,'Role','') in ('pcb','component','battery','insulator','lead_envelope')]
orig=parts+refs
for o in doc.Objects:
 if hasattr(o,'ViewObject'):o.ViewObject.Visibility=False
colors={'print':(.92,.94,.97),'pcb':(.04,.38,.18),'component':(.22,.25,.3),'battery':(.66,.70,.77),'insulator':(.8,.65,.2),'lead_envelope':(.8,.15,.07)}
copies=[]
for src in orig:
...
def capture(name,orientation='axon'):
 doc.recompute()
 if orientation=='axon':view.viewAxonometric()
...
for src,o in copies:
 if src in parts:
  o.Placement.Base.z+=36 if src==doc.TopAccess else (-24 if src.Role=='print' and src!=doc.Midframe else 0)
 else:o.Placement.Base.z+=(-12 if src.Name=='PackFloorInsulator' or src.Role=='battery' else (0 if src.Role=='lead_envelope' else 14))
capture('exploded')
for src,o in copies:o.Shape=src.Shape.copy();o.ViewObject.Visibility=False
...
for src,o in copies:
 o.Shape=src.Shape.common(Part.makeBox(20,60,60,A.Vector(-14.5,0,-20)));o.ViewObject.Visibility=not o.Shape.isNull()
capture('section','right')
# Frame-only plan shows open passage and opposite-side locators in separate underside view.
for src,o in copies:o.Shape=src.Shape.copy();o.ViewObject.Visibility=src==doc.Midframe
...
(H/'validation/renders.json').write_text(json.dumps({'result':'PASS','method':'Pinned FreeCAD GUI activeView BRep raster, authorized X11; not a physical prototype','images':['assembled','exploded','print-three','section','midframe-top','midframe-bottom'],'native_sha256':hashlib.sha256((H/'smove-r2-enclosure.FCStd').read_bytes()).hexdigest()},indent=2)+'\n')
