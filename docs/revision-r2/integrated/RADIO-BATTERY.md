...
| True OFF | With both USB and J2 pack disconnected, board has no external energy source. **Schematic ECO 2026-09-15 adds SW1**, an off-board toggle switch in series in the battery + lead (schematic-only until the PCB is updated); until then the fabricated board has **no latching master power switch**. Holding RESET only disables MCU; it does not disconnect charger/regulators/IMU. | No remote wake; with SW1 fitted, pack isolation becomes an external OFF action instead of a service procedure. |
...
**Schematic ECO 2026-09-15 (not yet on the fabricated board).** `U4` becomes **XC6220B331MR-G** (Torex, SOT-25, LCSC C86534) instead of AP2112K-3.3TRG1, and the charger's open-drain **CHG_N** now reaches the MCU (**U1 pad 20 = GPIO6**, pulled up by R24 100k to 3V3_MAIN). The chain stays **linear** — no switcher was added — but every published number for the 3.3V branch must be re-taken against the new part:
* Dropout is **20 mV @100 mA and 60 mV @300 mA (VOUT = 3.0 V test point, HS mode)** versus the AP2112's ~250 mV typical at 600 mA. The 3.3 V variant's own dropout is not the tested 3.0 V figure; treat the usable-range gain as **unmeasured** until the rundown test is repeated.
* Quiescent is **8 µA (PS) / 50 µA (HS)** with automatic GreenOperation mode switching (PS/HS switch current is a few mA, so a ~5 mA BLE standby sits near the mode boundary — expect mode-transition ripple and re-check IMU/RF noise there).
* SOT-25 standard-board dissipation is **250 mW**: keep `(VIN − VOUT) × IOUT` inside it, especially on USB with a depleted pack where SYS rises toward 4.4 V (a 360 mA burst would then exceed the limit and the thermal shutdown at 150 °C is the backstop).
* `CE` has **no internal pull-down on the B variant** — it must stay driven (it is tied to SYS here); do not float it when converting this pin into a load switch.
* The AP2112K remains fitted on the current PCB; this ECO is schematic-only until the board is updated (F8) and re-verified.
